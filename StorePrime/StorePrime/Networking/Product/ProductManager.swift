...
protocol ProductManagerProtocol {
    func getRandomProduct(completion: @escaping(Result<RandomProduct, GenericError>) -> Void)
    func getRandomProducts(amount: Int, completion: @escaping(Result<[RandomProduct], GenericError>) -> Void)
}
    ...
    func getRandomProduct(completion: @escaping (Result<RandomProduct, GenericError>) -> Void) {
        provider.request(.getRandomProduct) { result in
            ...
            case let .success(response):
                if let product = try? JSONDecoder().decode(RandomProduct.self, from: response.data) {
    ...
    func getRandomProducts(amount: Int, completion: @escaping(Result<[RandomProduct], GenericError>) -> Void) {
        provider.request(.getRandomProducts(amount: amount)) { result in
            ...
            case let .success(resopnse):
                if let products = try? JSONDecoder().decode([RandomProduct].self, from: resopnse.data) {
